...
server = function(input, output, session){
  ...
  ggplotly(
    ...
    geom_area() + 
    xlab("Year") +
    ylab("Forest loss in hectares (ha)") +
...
shinyApp(ui, server,options = list(width = 1100))
```
...
Overall it is expected that soy production will increase by % by the year 2030 and % by 2040. This could have a devastating impact on deforestation in Brazil and Argentina and other countries around the world. 
```{r echo=FALSE, message=FALSE, warning=FALSE}
ui =  fluidPage(
          theme = theme,
          sidebarLayout(
